...
function cleanup(store) {
	...
	for (var sessionId in sessions) {
		(function() {
			...
			var touchTime = session.touchTime;
			
			if ((expires && expires <= now) ||
					(touchTime != null && now - touchTime > maxAge)) {
 ...
 * @public
 */
function StreamStoryStore(opts) {
	MemoryStore.call(this)
	
	if (opts == null) opts = {};
	if (opts.maxAge == null) opts.maxAge = Number.MAX_VALUE;
...
StreamStoryStore.prototype.touch = function touch(sessionId, session, callback) {
	...
	if (currentSession) {
		...
		currentSession.touchTime = Date.now();
